...
# 六格坑加一顆踏腳石：全關唯一一個「走路過不去、衝刺過得去」的地方。
#
# 走路的滿跳水平距離是 5.1 格、衝刺是 7.65 格，而全關其他的坑都只有 3–4 格——
# 站著不動起跳（4.55 格）就全部過得去，於是衝刺、跳躍緩衝、土狼時間四個
# 手感機制在整張圖裡是零需求。這裡把坑拓成 6 格（384 px）：走路必須踩中間
# 那顆石頭分兩跳，按住 Shift 可以一口氣飛過去。
#
# 踏腳石放第 11 列（離地面 64 px），低到不會擋住視線，也低到跳上去毫無難度——
# 這個坑要考的是「要不要按 Shift」，不是精準跳躍。
bricks(107, 108, STAND)
...
# 箭頭移到磚台右緣外，不要插在金幣行列的正上方。
#
# 它的綠色是對的——跌停就是綠的，那是股市主題最直接的一個梗。問題在位置：
# 綠色＋向下箭頭在所有 UI 慣例裡都是「安全」「往這走」，而玩家剛在標題
# 看過「↓ 進水管」。把它排在一排金幣正中間，等於騙玩家跳過去。
put(177, STAND - 7, "a")
...
ground(253, 299)
put(258, STAND - 3, "?")
# 只有「按住跳鍵踩敵人」才拿得到的三枚金幣。
#
# 從地面滿跳，身體最高到 y=412；這排金幣在 y=352，差 60 px 就是搆不到。
# 踩住這隻小熊並按住跳鍵，回彈 276 px 之後身體涵蓋 321–441，剛好夠到。
# 不按住跳鍵的回彈只有 105 px，拿不到。
#
# 刻意做成可選的獎勵而不是必經路線：連踩爬高是給願意練的人的技巧，
# 不知道的人不該被擋在關卡外。
put(256, STAND, "b")
coins([255, 256, 257], STAND - 6)
...
header = "\n".join([
    "name: 盤面大道",
    # 時限 150 秒而不是 300。衝刺走完 300 格只要 45.7 秒、走路 68.6 秒——
    # 300 秒是實際需求的 6.6 倍，時鐘從頭到尾不咬人，於是 HUD 的時間欄、
    # 剩 30 秒的變色與警告音、通關的剩餘秒數紀錄全部永遠不會觸發。
    # 150 秒仍然寬鬆（走路走完還剩 81 秒），但警告真的會發生，而剩餘秒數
    # 也終於變成一條有比較意義的成績軸。
    "time: 150",
    "pipe1: level1_pipe_a",
    "---",
