...
# 水平平台橫跨在坑上方。
#
# 放第 9 列而不是第 8 列：第 8 列的站立面離地 240 px，而跳躍上限只有 236 px——
# 玩家從地面根本上不去，只能從它「通往」的那座磚台跳過去，整個因果是反的。
# 第 9 列離地 176 px，從坑左緣的地面跳得上，載到右邊剛好接上磚台。
put(198, STAND - 2, "P")
...
#
# 平台放第 10 列而不是第 7 列：第 7 列的站立面離地 304 px，而跳躍上限只有
# 236 px——玩家根本上不去，那兩枚金幣在正常遊玩下永遠拿不到。第 10 列的
# 站立面離地 112 px，上得去；行程往上 3 格之後頂端在第 7 列，再跳一次
# 就搆得到第 3 列的金幣。
put(230, STAND - 1, "V")
coins([230, 231], STAND - 7)
put(233, STAND, "b")
